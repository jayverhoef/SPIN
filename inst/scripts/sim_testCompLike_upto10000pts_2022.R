...
source(paste0(ScriptPath, 'geostatSim.R'))
source(paste0(ScriptPath, 'distGeoAni.R'))
source(paste0(ScriptPath, 'corModels.R'))
source(paste0(ScriptPath, 'addBreakColorLegend.R'))
source(paste0(ScriptPath, 'sim2DSinSurf.R'))
source(paste0(ScriptPath, 'cope.R'))
source(paste0(ScriptPath, 'fefe.R'))
source(paste0(ScriptPath, 'm2LLgprof.R'))
source(paste0(ScriptPath, 'm2LLg.R'))
...
if(n > 2000) {
  ...
  grpsize = round(25 + runif(1)*200)
	...
	colorpoints(xyDF$x, xyDF$y, xyz_4y$z, nclass = 8)
	colorpoints(xyDF$x, xyDF$y, xyz_4x$z, nclass = 8)
	colorpoints(x, y, Z, nclass = 8)


#-----------------------------------------------------------------------
#                       COPE
#-----------------------------------------------------------------------
  cat("\n", "estimation for sample size", n)
	...
	cope_rand = cope_simple(z ~ X1 + X2, data = d2, x_column = 'x', y_column = 'y', 
		subSampCol = 'grpindx.rand', thetaini = c(1, 1, .1))
	spcov_initial = spcov_initial('exponential', 
		range = cope_rand['range'], ie = cope_rand['ie'], 
		 de = cope_rand['de']) 
...
colorpoints = function(x, y, z, nclass, cex = 1)
{
	...
	cip = classIntervals(xyz_4x$z, n = 6, style = 'fisher')
	palp = viridis(6)
	cip_colors = findColours(cip, palp)
	plot(xyz_4x$x, xyz_4x$y, col = cip_colors, pch = 19, cex = 1.5)
