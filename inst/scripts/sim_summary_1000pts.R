library('spPart')
library('xtable')
data(scen1_1000pt_NNGP_est)
data(scen1_1000pt_NNGP_estse)
data(scen1_1000pt_NNGP_RMSPE)
data(scen1_1000pt_NNGP_PI90)
data(scen1_1000pt_NNGP_blk_pred)
data(scen1_1000pt_NNGP_blk_se)


scen1_1000pts_est_summ = data.frame(
	RMSE = sqrt(apply(scen1_1000pt_NNGP_est - 1,2,var)),
  CI90 = apply(scen1_1000pt_NNGP_est - qnorm(.95)*scen1_1000pt_NNGP_estse < 1 & 
	1 < scen1_1000pt_NNGP_est + qnorm(.95)*scen1_1000pt_NNGP_estse, 2, mean),
	CovParmEst = c('all', 'rand', NA, 'comp', NA, NA, 'zimm', NA, NA, 'NNGP', NA),
	FixEffEst = c('all', 'all', 'rand', 'all', 'rand', 'comp', 'all',
		'rand', 'zimm', 'crsp', 'rmspe')
)
...
scen1_1000pts_NNGP_pred_summ = data.frame(
	RMSPE = apply(scen1_1000pt_NNGP_RMSPE, 2, mean),
	PI90 = apply(scen1_1000pt_NNGP_PI90, 2, mean),
	CovParmEst = c('all', 'rand', NA, 'comp', NA, 'blnd', NA, 'NNGP', NA),
	PredMeth = c('all', 'all', 'nene', 'all', 'nene', 'all', 'nene', 'crsp', 'rmspe')
)
