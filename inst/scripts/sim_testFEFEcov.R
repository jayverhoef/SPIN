...
covbType_est1_gsim1000 = matrix(NA, nrow = nsim, ncol = 1)
covbType_est1se_gsim1000 = matrix(NA, nrow = nsim, ncol = 3)
covbType_est2_gsim1000 = matrix(NA, nrow = nsim, ncol = 1)
covbType_est2se_gsim1000 = matrix(NA, nrow = nsim, ncol = 3)
...
covbType_est1_gsim1000[iter] = cope_comp_fefe_rand$bhat[2]
covbType_est1se_gsim1000[iter,] = ses[2,]
covbType_est2_gsim1000[iter] = cope_comp_fefe_rand$bhat[3]
covbType_est2se_gsim1000[iter,] = ses[3,]

}

basepath = '/media/jay/data/2019_papers/fastREML/spPart_package/spPart/data/'
save(covbType_est1_gsim1000, file = paste0(basepath,'covbType_est1_gsim1000.rda'))
save(covbType_est1se_gsim1000, file = paste0(basepath,'covbType_est1se_gsim1000.rda'))
save(covbType_est2_gsim1000, file = paste0(basepath,'covbType_est2_gsim1000.rda'))
save(covbType_est2se_gsim1000, file = paste0(basepath,'covbType_est2se_gsim1000.rda'))

mean(covbType_est1_gsim1000)
covbT1se = covbType_est1se_gsim1000
covbT2se = covbType_est2se_gsim1000
covbT1se[is.na(covbT1se[,2]),2] =
  covbT1se[is.na(covbT1se[,2]),3] 
covbT2se[is.na(covbT2se[,2]),2] =
  covbT2se[is.na(covbT2se[,2]),3] 

mean(covbType_est1_gsim1000 - qnorm(.95)*covbT1se[,1] < 1 & 1 <
  covbType_est1_gsim1000 + qnorm(.95)*covbT1se[,1])
mean(covbType_est1_gsim1000 - qnorm(.95)*covbT1se[,2] < 1 & 1 <
  covbType_est1_gsim1000 + qnorm(.95)*covbT1se[,2])
mean(covbType_est1_gsim1000 - qnorm(.95)*covbT1se[,3] < 1 & 1 <
  covbType_est1_gsim1000 + qnorm(.95)*covbT1se[,3])
  
mean(covbType_est2_gsim1000 - qnorm(.95)*covbT2se[,1] < 1 & 1 <
  covbType_est2_gsim1000 + qnorm(.95)*covbT2se[,1])
mean(covbType_est2_gsim1000 - qnorm(.95)*covbT2se[,2] < 1 & 1 <
  covbType_est2_gsim1000 + qnorm(.95)*covbT2se[,2])
mean(covbType_est2_gsim1000 - qnorm(.95)*covbT2se[,3] < 1 & 1 <
  covbType_est2_gsim1000 + qnorm(.95)*covbT2se[,3])

mean(covbType_est1_gsim1000 - 
  qnorm(.95)*sqrt((covbT1se[,2]^2+covbT1se[,3]^2)/2)  < 1 & 
  1 < covbType_est1_gsim1000 + 
  qnorm(.95)*sqrt((covbT1se[,2]^2+covbT1se[,3]^2)/2))

mean(covbType_est2_gsim1000 - 
  qnorm(.95)*sqrt((covbT2se[,2]^2+covbT2se[,3]^2)/2)  < 1 & 
  1 < covbType_est2_gsim1000 + 
  qnorm(.95)*sqrt((covbT2se[,2]^2+covbT2se[,3]^2)/2))
