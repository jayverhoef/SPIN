...
#' Predictions for partitioned spatial linear models using global estimate of fixed effects
#'
#' Predictions for partitioned spatial linear models using global estimate of fixed effects
#'
...
pulo1 <- function(theta, betahat, covb, z, X, Xp, xcoords, ycoords, xcoordsp,
  ycoordsp, predmeth = 'nearnei', nNN = 50, par = FALSE)
{
		  ...
		  preds <- matrix(NA, nrow = length(xcoordsp), ncol = 2)
			for(i in 1:length(xcoordsp)) {
				...
				preds[i,1] = sum(as.vector((Vi %*% zi)) * Vpred) +
					Xp[i,] %*% betahat - t(Vpred) %*% Vi %*% Xi %*% betahat
				preds[i,2] = sqrt(sill -
					sum((Vi %*% Vpred) * Vpred) +
